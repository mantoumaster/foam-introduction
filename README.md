...
2.  **AgentId**：在应用详情页上方可以看到 `AgentId`（数字，如 `1000006`）。
3.  **Secret**：点击 `Secret` 旁的 **查看** 链接 -> 手机企业微信扫码 -> 获得一串长字符串（如 `J4VlAzNnc8...`）。

### 🔐 **3. Token & EncodingAESKey**
> 📡 这是用于接收消息和加密通讯的配置。

1.  **路径**：在同一个应用的详情页 -> 向下滚动找到 **接收消息** -> 点击 **设置 API 接收**。
2.  **Token**：点击 **随机获取** 按钮，或者自己填写（如 `WAUBY9...`）。
3.  **EncodingAESKey**：点击 **随机获取** 按钮（如 `XDxWQR...`）。
